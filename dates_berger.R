### Dates pour survie dépendantes du temps sans amputations ###
...
berger$date_amput_deces15<-ifelse(berger$delai_amput_deces<=15,as.numeric(berger$date_amput_deces),as.numeric(berger$date_diag) +365*15)
...
dates<-berger[,c("nip","date_diag","date_colchi","date_fin_colchi","date_asp","date_fin_asp","date_pla",
                 ...
                 "date_gv1","date_gv2","date_gv3","date_gv4","date_gv5","date_amput_deces15","amput_deces15")]

for(i in c("date_diag","date_colchi","date_fin_colchi","date_asp","date_fin_asp","date_pla",
          "date_fin_pla","date_coag","date_fin_coag","date_statin","date_fin_statin",
          "date_inh","date_fin_inh","date_vas","date_fin_vas","date_iec","date_fin_iec",
          "date_gv1","date_gv2","date_gv3","date_gv4","date_gv5","date_amput_deces15") ){
...
for(i in c("date_colchi","date_asp","date_pla",
  ...
  #berger[,i]<-ifelse (berger[,i]<=berger$date_diag,berger$date_diag ,berger[,i])
  dates[,i]<-ifelse (dates[,i]>=dates$date_amput_deces15,NA ,dates[,i])
}
...
for(i in c("date_fin_colchi","date_fin_asp",
  ...
  #berger[,i]<-ifelse (berger[,i]<=berger$date_diag,berger$date_diag ,berger[,i])
  dates[,i]<-ifelse (dates[,i]>dates$date_amput_deces15,dates$date_amput_deces15 ,dates[,i])
}
PERS<-NULL
j=55
...
statut <-dates[dates$nip==j, c("date_amput_deces15","amput_deces15")]
  ...
  personnec<-merge(personnec,statut,by.x="personneb",by.y="date_amput_deces15",all.x=TRUE)
  personnec$amput_deces15<-ifelse(is.na(personnec$amput_deces15),0, personnec$amput_deces15)
  ...
  #                                                      ,"v4","v5","amput_deces15","revas")]))
  
  
   personned<-subset(personnec,!duplicated(personnec[,c("personneb","tt","amput_deces15","revas")]))
  ...
  for (j in 1: nrow(personned)-1){
    ...
    personned$daf15[j]<-personned$amput_deces15[j+1]
  ...
  PERS<-rbind(PERS,personned)
}


PERS<-PERS[!is.na(PERS$daf15),]
...
result.coxt <- function (modele)
{
  ...
  res$Variable<- gsub("PERS\\[, i\\]","",res$Variable)
...
for (i in c( "revas","tt")
  ...
  mo<-coxph( Surv(time=as.numeric(start), time2=as.numeric(end), event=daf15)~PERS[,i],data=PERS)  
